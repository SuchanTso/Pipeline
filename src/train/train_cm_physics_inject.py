...
def train_one_epoch_physics_injection(
    ...
    for batch_data in pbar:
        ...
        for _ in range(batch_pairs):
            ...
            g_phys_p_proj = g_phys_p
            g_phys_q_proj = g_phys_q
...
if __name__ == "__main__":
    ...
    finetune_pressure_model = 'model/water_gunet_finetuned_cm_pressure_model_best.pt'
    finetune_flow_model = 'model/water_gunet_finetuned_cm_flow_model_best.pt'
    ...
    # 使用较小的学习率
    optimizer_p = torch.optim.AdamW(model_p.parameters(), lr=1e-5)
    optimizer_q = torch.optim.AdamW(model_q.parameters(), lr=1e-5)
